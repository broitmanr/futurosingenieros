...
async function enviarNotificacionesVencimientoEntrega() {
    ...
    // Establecer la fecha de vencimiento a 3 días a partir de hoy
    const fechaVencimiento = new Date(fechaActual);
    fechaVencimiento.setDate(fechaActual.getDate() + 3);

    // Asegúrate de que las fechas se almacenen en UTC
    ...
    try {
        ...
        for (const entrega of entregasProximasAVencer) {
            ...
            if (alumnosSinEntrega.length > 0) {
                const mensaje = `La entrega pactada "${entrega.nombre}" vence en 3 días. Recuerda completarla a tiempo.`;
